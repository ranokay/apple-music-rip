...
function saveDownloadHistory(history: Record<string, unknown>) {
  try {
    writeFileSync(DOWNLOAD_HISTORY_FILE, JSON.stringify(history, null, 2), "utf-8");
    return true;
  } catch (error) {
...
function wrapperHostPort() {
  ...
  try {
    ...
    appendLog(wrapperLogs, `⚠️ Could not read config.yaml for wrapper port: ${error}`);
  }
...
function updateConfigForFormat(formatChoice: string, downloadMode: string, isAtmos: boolean) {
  try {
    if (existsSync(CONFIG_PATH)) {
      const raw = readFileSync(CONFIG_PATH, "utf-8");
      const config = (yaml.load(raw) as Record<string, unknown>) || {};
      ...
      if (downloadMode === "lyrics") {
        saveFolder = (config["alac-save-folder"] as string) || "AM-DL downloads";
      } else if (downloadMode === "covers") {
        saveFolder = (config["alac-save-folder"] as string) || "AM-DL downloads";
      } else if (formatChoice === "atmos") {
        saveFolder = (config["atmos-save-folder"] as string) || "AM-DL-Atmos downloads";
      } else if (formatChoice === "aac") {
        saveFolder = (config["aac-save-folder"] as string) || "AM-DL-AAC downloads";
      } else {
        saveFolder = (config["alac-save-folder"] as string) || "AM-DL downloads";
      }
...
async function runSingleDownload(
  ...
  if (selectedTracks) {
    cmd = [cmd[0], cmd[1], cmd[2], "--select-tracks", selectedTracks, ...cmd.slice(3)];
  ...
  try {
    ...
    if (!runningProcess) {
      appendLog(downloaderLogs, `❌ ${formatChoice.toUpperCase()} download failed to start.`);
      return false;
    }
    ...
    await Promise.all([consume(runningProcess.stdout), consume(runningProcess.stderr)]);
    const exitCode = await closePromise;

    if (exitCode === 0) {
      appendLog(downloaderLogs, `✅ ${formatChoice.toUpperCase()} download completed!`);
      return true;
    }
    ...
    appendLog(downloaderLogs, `❌ Error during ${formatChoice} download: ${error}`);
    return false;
  } finally {
...
async function runMultiDownload(
  ...
  const sortedFormats = [...formats].sort((a, b) => (a === "atmos" ? -1 : b === "atmos" ? 1 : 0));
  ...
  for (let i = 0; i < sortedFormats.length; i += 1) {
    ...
    appendLog(downloaderLogs, `📥 Format ${i + 1}/${totalFormats}: ${fmt.toUpperCase()}`);
    ...
    const success = await runSingleDownload(link, fmt, downloadMode, selectedTracks, isAtmos);
  ...
  currentProgress = { label: "✅ All downloads completed", percent: 100, details: "" };
...
async function runCommand(cmd: string[], options: Record<string, unknown>, timeoutMs: number) {
...
app
  ...
  .post("/api/preview", async ({ body }) => {
    ...
    try {
      const result = await runCommand(["go", "run", "main.go", "--preview", link], {
        cwd: AMD_DIR,
        env,
      }, 60_000);
      ...
      if (result.code !== 0) {
        return {
          status: "error",
          msg: stderr || stdout || `Preview failed with exit code ${result.code}`,
        };
      }
  ...
  .post("/api/download", async ({ body }) => {
    ...
    const formats = Array.isArray(rawFormats)
      ...
      : typeof rawFormats === "string"
        ? rawFormats.split(",").map((format) => format.trim()).filter(Boolean)
        : [];
    ...
    runMultiDownload(link, formatsList, mode, selectedTracks, metadata).catch((error) => {
  ...
  .post("/api/save-config", ({ body }) => {
    try {
      ...
      const pathFields = new Set(["alac-save-folder", "atmos-save-folder", "aac-save-folder"]);
      ...
      Object.entries(configData).forEach(([key, value]) => {
        if (integerFields.has(key)) {
          ...
          if (typeof value === "string") {
            configData[key] = ["true", "1", "yes", "on"].includes(value.toLowerCase());
          } else {
    ...
    try {
      ...
      return {
        ...
        folders: {
          ...
          atmos: (config["atmos-save-folder"] as string) || "AM-DL-Atmos downloads",
...
if (existsSync(DIST_DIR)) {
  ...
  app.get("/settings", () => Bun.file(path.join(DIST_DIR, "settings", "index.html")));
